...
"""
Optimization Script - Version 3.6
Objective: Hybrid Soft Time Penalty Model
Weights: Cost α = 0.4, Time β = 0.6
"""
...
# Build model
model = LpProblem("Transport_Optimization_v3_6_Hybrid", LpMinimize)
...
# Objective: soft time penalty with higher time weight
alpha, beta = 0.4, 0.6
model += (
    lpSum(
        ...
        for _, row in transport_df.iterrows()
        if (row['Product'], row['From'], row['To']) in x
    ) +
...
# Solve
print("Solving model v3.6 (soft time penalty)...")
model.solve()
...
pd.DataFrame(results).to_excel("optimized_transport_plan_v3_6.xlsx", index=False)
pd.DataFrame(unmet_summary).to_excel("unmet_demand_report_v3_6.xlsx", index=False)
print("Results saved.")
